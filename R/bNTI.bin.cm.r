bNTI.bin.cm<-function(comm, meta.group=NULL,meta.spool=NULL,
                      ...
                      exclude.conspecifics = FALSE, dirichlet = FALSE)
{
  ...
  bMNTD.random<-function(i,pd.desc, pd.spname, pd.wd, pdid.bin,
                         comms,sp.bin,bin.num,bin.lev,weighted,
  ...
  bMNTD.rand<-parallel::parLapply(c1,1:rand,bMNTD.random,pd.desc=pd.desc, pd.spname=pd.spname,
                        pd.wd=pd.wd, pdid.bin=pdid.bin, comms=comms,sp.bin=sp.bin,
                        bin.num=bin.num,bin.lev=bin.lev,weighted=weighted,
                        unit.sum=unit.sum,exclude.conspecifics=exclude.conspecifics,
    ...
    res.nn=lapply(1:length(res.n),
                 function(i)
                 {
                   ...
                   correct.spec(commi = com.bin[[i]],resulti = res.n[[i]],
                                pdi = pdi, sdmi=sdm[,,i],
                                meta.spool.bini = meta.spool.bini,
                                meta.freq.bini = meta.freq.bini,
                                meta.ab.bini = meta.ab.bini)
                 })
    if(detail.null)
    {
